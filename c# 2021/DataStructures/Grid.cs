...
public record Grid<T>(IReadOnlyList<IReadOnlyList<Cell<T>>> Cells) : IEnumerable<Cell<T>>
{
    ...
    public static Grid<T> FromChars(IEnumerable<IEnumerable<char>> chars, Func<char, T> parseFunc)
    {
        var cells = chars.Select((yChars, y) => yChars.Select((xChar, x) => new Cell<T>(x, y, parseFunc(xChar))).ToList()).ToList();
