...
namespace AdventOfCode.Solutions.Year2017
{
	public class Day12 : IPuzzleSolution
	{
		...
		private Regex _r = new Regex(@"^(?<d1>\d+) <-> (?<nb>[\d, ]+)$");
		...
		private Dictionary<string, Prog> ParseInput(string[] input)
		{
			return input
				.Select(x => _r.Match(x))
		...
		private static int GroupSize(Prog root, Dictionary<string, Prog> progs, int groupId)
		{
